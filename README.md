# Hi, I'm Navonil Mandal 👋

### 🚀 AI/ML Enthusiast | CSE Student (BIT Mesra, 2027) | Building Intelligent Systems

I love turning ideas into real, working AI systems. From deep learning to automation, I enjoy experimenting with complex problems and creating production-ready solutions.

---

## 🧠 **About Me**

* 🎓 CSE Undergrad (BIT Mesra, 2027 batch)
* 🤖 Passionate about AI, ML, Data Science, Data Analyst, DSA
* 🔥 Always curious, always building
* 🎨 Love blending tech with creativity (video gen, Ghibli AI, deepfake detectors, ADAS)
* 🎮 Gamer | 🎵 Musician | ✍️ Writer

---

## 🛠️ **Tech Stack**

**Programming:** Python, C++, JavaScript, Java, SQL

![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?logo=c%2B%2B&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=black)


**ML/DL:** TensorFlow, PyTorch, scikit-learn, OpenCV, YOLOV8n

![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?logo=tensorflow&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?logo=pytorch&logoColor=white)
![Scikit-Learn](https://img.shields.io/badge/Scikit--Learn-F7931E?logo=scikitlearn&logoColor=white)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?logo=opencv&logoColor=white)


**Tools:** Git, Colab, Jupyter, Docker

![Git](https://img.shields.io/badge/Git-F05032?logo=git&logoColor=white)
![Colab](https://img.shields.io/badge/Google%20Colab-F9AB00?logo=googlecolab&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?logo=jupyter&logoColor=white)


**Cloud:** AWS, GCP (beginner)

---

## 📌 **Top Projects**

### 🔹 **ADAS – Advanced Driver Assist System**

Real-time lane detection, vehicle detection, and driver safety analysis.

🔗 GitHub: [https://github.com/navonilmandal/adas_ongoing](https://github.com/navonilmandal/adas_ongoing)

**Tech Used:**  
![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)  
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?logo=opencv&logoColor=white)  
![NumPy](https://img.shields.io/badge/NumPy-013243?logo=numpy&logoColor=white)  
![Computer Vision](https://img.shields.io/badge/Computer%20Vision-00A8E8)



---

### 🔹 **Deepfake Detector**

CNN-based architecture for detecting manipulated facial videos with preprocessing + augmentation.

🔗 GitHub: https://github.com/navonilmandal/deepfake_video_detection_project

### 🔹 Deepfake Detector  

**Tech Used:**  
![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)  
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?logo=pytorch&logoColor=white)  
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?logo=opencv&logoColor=white)  
![CNN](https://img.shields.io/badge/CNN-FF6F00)




---

### 🔹 **Quad-Tree Image Compressor**

A unique image compression algorithm built using dynamic quadtree partitioning.

🔗 GitHub: https://github.com/navonilmandal/quadtree_compressor

### 🔹 Quad-Tree Image Compressor  

**Tech Used:**  
![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)  
![Algorithm](https://img.shields.io/badge/Algorithm-00B8D4)  
![Image Processing](https://img.shields.io/badge/Image%20Processing-8E44AD)



---



## 🌍 **Connect With Me**

* 📧 Email: navonilmandal@gmail.com
* 🔗 LinkedIn: https://www.linkedin.com/in/navonilmandal/



## 🐍 Contribution Snake
![snake animation](https://raw.githubusercontent.com/navonilmandal/navonilmandal/gh-pages/snake.svg)



---

### ⭐ *If you like my work, consider starring my repositories!*
