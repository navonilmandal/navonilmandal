# Hi, I'm Navonil Mandal 👋

### 🚀 AI/ML Engineer | GenAI & LLM Systems | CSE @ BIT Mesra '27 | Building Production-Ready Intelligent Systems

I love turning ideas into real, working AI systems — from persistent cognitive AI platforms to deepfake detectors and adaptive traffic controllers. I enjoy solving complex problems and shipping production-ready solutions.

---

## 🧠 About Me

* 🎓 B.Tech CSE @ Birla Institute of Technology, Mesra (2027 batch) — GPA: 8.1/10
* 🤖 Passionate about AI/ML, GenAI & LLM Engineering, Data Science, Computer Vision, and NLP
* 🔬 ML Researcher under DR. Rayees Ahmad — Intelligent Traffic Signal Control using RL & metaheuristics
* 🔥 Always curious, always building production-grade AI systems
* 🎮 Gamer | 🎵 Musician | ✍️ Writer

---

## 🛠️ Tech Stack

**Programming:**
![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?logo=c%2B%2B&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?logo=openjdk&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?logo=mysql&logoColor=white)

**ML / DL / CV:**
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?logo=pytorch&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?logo=tensorflow&logoColor=white)
![Scikit-Learn](https://img.shields.io/badge/Scikit--Learn-F7931E?logo=scikitlearn&logoColor=white)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?logo=opencv&logoColor=white)
![XGBoost](https://img.shields.io/badge/XGBoost-FF6600?logo=xgboost&logoColor=white)

**GenAI / LLM:**
![Gemini](https://img.shields.io/badge/Gemini-8E75B2?logo=google&logoColor=white)
![Ollama](https://img.shields.io/badge/Ollama-000000?logo=ollama&logoColor=white)
![Qdrant](https://img.shields.io/badge/Qdrant-DC244C?logo=qdrant&logoColor=white)
![LangChain](https://img.shields.io/badge/RAG_Pipelines-1C3C3C?logoColor=white)

**Backend & Deployment:**
![FastAPI](https://img.shields.io/badge/FastAPI-009688?logo=fastapi&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?logo=flask&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?logo=postgresql&logoColor=white)
![JWT](https://img.shields.io/badge/JWT-000000?logo=jsonwebtokens&logoColor=white)

**Data & Visualization:**
![Pandas](https://img.shields.io/badge/Pandas-150458?logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?logo=numpy&logoColor=white)
![Power BI](https://img.shields.io/badge/Power_BI-F2C811?logo=powerbi&logoColor=black)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557c?logoColor=white)

**Tools:**
![Git](https://img.shields.io/badge/Git-F05032?logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?logo=github&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?logo=jupyter&logoColor=white)
![Google Colab](https://img.shields.io/badge/Google%20Colab-F9AB00?logo=googlecolab&logoColor=white)

---

## 📌 Featured Projects

### 🔹 [APJ AI: A Cognitive Workspace](https://github.com/navonilmandal/APJ-AI-A-Cognitive-Workspace)
A **persistent cognitive GenAI platform** — not a chatbot wrapper. Features semantic memory (Qdrant), reflection-based reasoning, hybrid LLM routing (Ollama ↔ Gemini/Groq), identity-aware grounding, and full Docker Compose deployment with JWT auth and PostgreSQL.

![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?logo=fastapi&logoColor=white)
![Qdrant](https://img.shields.io/badge/Qdrant-DC244C?logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white)
![Gemini](https://img.shields.io/badge/Gemini-8E75B2?logo=google&logoColor=white)

---

### 🔹 [ChurnSense: E-commerce Customer Retention System](https://github.com/navonilmandal/ChurnSense-E-commerce-Customer-Retention-System)
End-to-end ML pipeline for churn prediction — EDA, CLV feature engineering, XGBoost tuning, 4-tier customer segmentation, A/B testing (p < 0.05), and Power BI dashboard.

![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)
![XGBoost](https://img.shields.io/badge/XGBoost-FF6600?logoColor=white)
![Scikit-Learn](https://img.shields.io/badge/Scikit--Learn-F7931E?logo=scikitlearn&logoColor=white)
![Power BI](https://img.shields.io/badge/Power_BI-F2C811?logo=powerbi&logoColor=black)

---

### 🔹 [Deepfake Video Detection System](https://github.com/navonilmandal/deepfake_video_detection_project)
Hybrid **CNN–LSTM** architecture for deepfake detection — MTCNN face extraction, FaceNet embeddings, temporal smoothing, and 84% accuracy on the test split.

![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?logo=pytorch&logoColor=white)
![CNN-LSTM](https://img.shields.io/badge/CNN--LSTM-FF6F00?logoColor=white)
![MTCNN](https://img.shields.io/badge/MTCNN-5C3EE8?logoColor=white)

---

### 🔹 [Intelligent Traffic Signal Control](https://github.com/navonilmandal/intelligent-traffic-control)
Adaptive traffic system using **SUMO + TraCI** with PSO & Genetic Algorithm optimisation — improved average vehicle speed from **1.25 m/s → 10 m/s**. Currently extending with a Reinforcement Learning controller under Dr. Rayees Ahmad, BIT Mesra.

![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)
![SUMO](https://img.shields.io/badge/SUMO-TraCI-00599C?logoColor=white)
![RL](https://img.shields.io/badge/Reinforcement_Learning-1D9E75?logoColor=white)

---

## 🌍 Connect With Me

* 📧 navonilmandal@gmail.com
* 🔗 [linkedin.com/in/navonilmandal](https://www.linkedin.com/in/navonilmandal/)
* 💻 [github.com/navonilmandal](https://github.com/navonilmandal)

---

## 🐍 Contribution Snake

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/navonilmandal/navonilmandal/refs/heads/output/github-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/navonilmandal/navonilmandal/refs/heads/output/github-snake.svg" />
  <img alt="github contribution snake animation" src="https://raw.githubusercontent.com/navonilmandal/navonilmandal/refs/heads/output/github-snake.svg" />
</picture>

---

⭐ *If you find my work useful, consider starring the repositories!*
